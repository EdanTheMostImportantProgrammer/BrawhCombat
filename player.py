...
class Character:
    def __init__(self, x, y, speed_x, speed_y, jumps, color, health_bar_x, health_bar_y, energy_bar_x,
        ...
        self.heavy_attack_prep = False
        self.heavy_attacking = False
        self.damage_multiplier = 1
        self.heavy_attack_time = 0
    ...
    def bump(self, target):
        damage = (abs(self.speed_x) + abs(self.speed_y)) - (abs(target.speed_x) + abs(target.speed_y)) * self.damage_multiplier
        if damage > 0:
    ...
    def heavy_attack(self):
        ...
        target = pygame.math.Vector2(mx, my)
        start = pygame.math.Vector2(self.x, self.y)
        self.direction = (target - start).normalize() * 20
        self.speed_x += self.direction.x
        self.speed_y = self.direction.y
        self.heavy_attacking = False

    def heavy_attack_check(self):
        if self.heavy_attack_prep:
            self.speed_x = 0
            self.speed_y = 0
            self.heavy_attack_time += 1
            if self.heavy_attack_time >= 40:
                self.heavy_attack()
                self.heavy_attack_prep = False
                self.heavy_attack_time = 0
        else:
